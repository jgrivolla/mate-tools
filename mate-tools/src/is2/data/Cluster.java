...
final public class Cluster {
	...
	public Cluster(String clusterFile, IEncoderPlus mf, int ls) {
		...
		word2path = new short[mf.getFeatureCounter().get(PipeGen.WORD)][1];
		...
		try {
			...
			while ((line =inputReader.readLine())!=null) {
				...
				word2path[wd][0] = (short)mf.getValue(LPATH, split[0].length()<ls?split[0]:split[0].substring(0,ls));
			}
			inputReader.close();
			int fill=0;
	...
	public Cluster(DataInputStream dis) throws IOException {
		...
		word2path = new short[dis.readInt()][1];
